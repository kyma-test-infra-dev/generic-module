...
public final class ConnectivityProxyCASecret extends ConnectivityProxyDependentResource<Secret> {

	private static final Logger LOGGER = LogManager.getLogger(ConnectivityProxyCASecret.class);

	private byte[] connectivityCACert;

	public ConnectivityProxyCASecret(KubernetesClient kubernetesClient) {
	...
	protected String getDependentResourceName(ConnectivityProxy connectivityProxy) {
		return connectivityProxy.caSecretName();
	}
	...
	private Map<String, String> createData(ConnectivityProxy connectivityProxy) {
		return Map.of(connectivityProxy.caSecretDataKey(), "");
	}
