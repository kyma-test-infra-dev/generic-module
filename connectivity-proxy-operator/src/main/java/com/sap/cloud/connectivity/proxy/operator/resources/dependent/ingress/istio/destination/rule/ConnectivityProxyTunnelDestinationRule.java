...
/**
 * An Istio DestinationRule, which sets a connection timeout to the tunnel service to the replica with ordinal zero. The
 * value is taken from a property in the custom resource.
 */
public final class ConnectivityProxyTunnelDestinationRule extends IstioDependentResource<DestinationRule> {

	public ConnectivityProxyTunnelDestinationRule(KubernetesClient kubernetesClient) {
	...
	private DestinationRuleSpec createSpec(ConnectivityProxy connectivityProxy) {
		return new DestinationRuleSpecBuilder()
				.withHost(connectivityProxy.tunnelServiceZeroFQNHost())
